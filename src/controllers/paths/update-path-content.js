...
export default function makeUpdatePathContentController({ updatePathContent }) {
  return async function updatePathContentController(httpRequest) {
    try {
      // validate auth token
      ...
      // call action
      const httpResult = await updatePathContent(httpRequest)
      return httpResult
    } catch (e) {
      return makeHttpError({
